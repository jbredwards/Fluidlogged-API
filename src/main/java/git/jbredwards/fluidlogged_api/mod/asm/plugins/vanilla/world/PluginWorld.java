...
public final class PluginWorld implements IASMPlugin
{
    ...
    public boolean transformClass(@Nonnull ClassNode classNode, boolean obfuscated) {
         ...
         * {
         *     // Make sure to call "this.getChunk(pos)" here, to get around forge
         *     // falsely labeling this mod as the cause for cascading world gen.
         *     return Hooks.setBlockToAir(this.getChunk(pos), pos, 3);
         * }
         */
        overrideMethod(classNode, method -> method.name.equals(obfuscated ? "func_175698_g" : "setBlockToAir"),
            "setBlockToAir", "(Lnet/minecraft/world/chunk/Chunk;Lnet/minecraft/util/math/BlockPos;I)Z", generator -> {
                generator.visitVarInsn(ALOAD, 0);
                generator.visitVarInsn(ALOAD, 1);
    ...
    public static final class Hooks
    {
        ...
        public static IBlockState isFlammableFluidWithin(@Nonnull IBlockState state, @Nonnull World world, @Nonnull BlockPos pos, @Nonnull AxisAlignedBB bb) {
            if(state.getBlock() == Blocks.FIRE || state.getBlock().isAir(state, world, pos)) return state;
            else if(FluidCollisionHandler.isAABBInsideMaterial(state, world, pos, bb, Material.LAVA)) return Blocks.FIRE.getDefaultState();
            else if(FluidloggedUtils.isFluid(state)) return Blocks.AIR.getDefaultState();
        ...
        public static boolean setBlockToAir(@Nonnull final World world, @Nonnull final BlockPos pos, @Nonnull final IBlockState airState, final int blockFlags) {
            return setBlockToAir(world.getChunk(pos), pos, blockFlags);
        }
