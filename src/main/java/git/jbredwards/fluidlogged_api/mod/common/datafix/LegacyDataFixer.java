...
public final class LegacyDataFixer
{
    ...
    public static NBTTagCompound fix(@Nonnull NBTTagCompound compound) {
        ...
        if(!FLUID_MAPPINGS.isEmpty()) {
            ...
            final Int2ObjectMap<List<Pair<BlockPos, LegacyDataGetter>>> legacyTiles = new Int2ObjectOpenHashMap<>();
            final Map<String, Function<NBTTagCompound, LegacyDataGetter>> legacyDataGetters = ImmutableMap.of(
                    "fluidlogged_api:te", FluidloggedAPIDataAdapter::new,
                    "njarm:waterlogged_te", NJARM2DataAdapter::new);
            for(Iterator<NBTBase> it = tileEntities.iterator(); it.hasNext();) {
                ...
                final Function<NBTTagCompound, LegacyDataGetter> dataGetter = legacyDataGetters.get(nbt.getString("id"));
                if(dataGetter != null) {
                    ...
                    @Nullable List<Pair<BlockPos, LegacyDataGetter>> list = legacyTiles.get(sectionY);
                    if(list == null) {
                        list = new ArrayList<>();
                        legacyTiles.put(sectionY, list);
                    }
                    //assigns fluidlogged te's to a section
                    list.add(Pair.of(pos, dataGetter.apply(nbt)));
            ...
            if(!legacyTiles.isEmpty()) {
                final NBTTagList cap = FluidloggedAPIFixableData.getOrCreateFluidCapabilityData(level);
                ...
                for(int i = 0; i < sections.tagCount(); ++i) {
                    ...
                    final @Nullable List<Pair<BlockPos, LegacyDataGetter>> sectionLegacyTiles = legacyTiles.get(sectionY);
                    if(sectionLegacyTiles != null) {
                        ...
                        for(final Pair<BlockPos, LegacyDataGetter> entry : sectionLegacyTiles) {
                            ...
                            @Nullable Block containedBlock = Block.getBlockFromName(entry.getValue().getContainedBlock());
                            int containedMeta = entry.getValue().getContainedMetadata();
    ...
    // get data from old fluidlogged api data format
    static class FluidloggedAPIDataAdapter implements LegacyDataGetter
    {
        @Nonnull
        final NBTTagCompound stored;
        FluidloggedAPIDataAdapter(@Nonnull final NBTTagCompound nbtIn) { stored = nbtIn.getCompoundTag("Stored"); }

        @Nonnull
        @Override
        public String getContainedBlock() { return stored.getString("id"); }

        @Override
        public int getContainedMetadata() { return stored.getInteger("meta"); }
    }

    // get data from old njarm2 data format
    static class NJARM2DataAdapter implements LegacyDataGetter
    {
        @Nonnull
        final NBTTagCompound nbt;
        NJARM2DataAdapter(@Nonnull final NBTTagCompound nbtIn) { nbt = nbtIn; }

        @Nonnull
        @Override
        public String getContainedBlock() { return nbt.getString("StoredId"); }

        @Override
        public int getContainedMetadata() { return nbt.getInteger("StoredMeta"); }
    }
}
