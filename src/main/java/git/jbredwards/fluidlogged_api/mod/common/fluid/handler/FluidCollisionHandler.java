...
public final class FluidCollisionHandler
{
    ...
    public static IBlockState getStateAtViewpoint(@Nonnull final IBlockState state, @Nonnull final IBlockAccess world, @Nonnull final BlockPos pos, @Nonnull final Vec3d viewpoint) {
        ...
        if(isWithinFluid(world, pos, viewpoint.x, viewpoint.y, viewpoint.z, state, viewpointEntity.getEntityBoundingBox(), viewpointEntity, false)) return state;
    ...
    public static Boolean isEntityInsideMaterial(@Nonnull final IBlockAccess worldIn, @Nonnull final BlockPos pos, @Nonnull final IBlockState state, @Nonnull final Entity entity, final double yToTest, @Nonnull final Material materialIn, final boolean testingHead) {
        ...
        else return testingHead ? Boolean.valueOf(isWithinFluid(worldIn, pos, entity.posX, yToTest, entity.posZ, state, entity.getEntityBoundingBox().shrink(0.001), entity, true)) : isAABBInsideLiquid(worldIn, pos, entity.getEntityBoundingBox().shrink(0.001), state, entity, true);
    }
    ...
    static Boolean isAABBInsideLiquid(@Nonnull final IBlockAccess worldIn, @Nonnull final BlockPos pos, @Nonnull final AxisAlignedBB bb, @Nullable final IBlockState stateIn, @Nullable final Object entity, final boolean checkCache) {
        if(bb.minX > pos.getX() + 1 || bb.maxX < pos.getX() || bb.minZ > pos.getZ() + 1 || bb.maxZ < pos.getZ()) return Boolean.FALSE; // don't collide with fluid if BB doesn't horizontally intersect
            ...
            return isYWithinFluidEstimate(world, pos, bb.minY, bb.maxY, stateIn != null ? stateIn : FluidloggedUtils.getFluidOrReal(world, pos), checkCache);
        }
        ...
        return !(extendedState instanceof IExtendedBlockState) ? isYWithinFluidEstimate(access, pos, bb.minY, bb.maxY, state, checkCache) :
                isPointWithinFluid(pos, bb.minX, bb.minY, bb.maxY, bb.minZ, (IExtendedBlockState)extendedState, checkCache)
                        || isPointWithinFluid(pos, bb.minX, bb.minY, bb.maxY, bb.maxZ, (IExtendedBlockState)extendedState, checkCache)
                        || isPointWithinFluid(pos, bb.maxX, bb.minY, bb.maxY, bb.minZ, (IExtendedBlockState)extendedState, checkCache)
                        || isPointWithinFluid(pos, bb.maxX, bb.minY, bb.maxY, bb.maxZ, (IExtendedBlockState)extendedState, checkCache);
    }
    ...
    static boolean isWithinFluid(@Nonnull final IBlockAccess worldIn, @Nonnull final BlockPos pos, final double x, final double y, final double z, @Nullable final IBlockState stateIn, @Nonnull final AxisAlignedBB bb, @Nullable final Entity entity, final boolean checkCache) {
        if(FluidloggedAPIConfig.fancyFluidEntityCollision.test(bb, entity) && IWorldProvider.getWorld(worldIn).isAreaLoaded(pos.add(-1, -1, -1), pos.add(1, 1, 1))) {
            ...
            return extendedState instanceof IExtendedBlockState ? isPointWithinFluid(pos, x, y, y, z, (IExtendedBlockState)extendedState, checkCache) : isYWithinFluidEstimate(access, pos, y, y, state, checkCache);
        }

        else {
            @Nonnull final IBlockAccess access = worldIn instanceof World ? new FluidCache(worldIn, pos, 0, 1) : worldIn;
            return isYWithinFluidEstimate(access, pos, y, y, stateIn != null ? stateIn : FluidloggedUtils.getFluidOrReal(access, pos), checkCache);
        }
    }
