...
public interface IFluidUpdateHelper extends ISpecializedFluidNeighborInfo
{
    class Forge extends SpecializedFluidNeighborInfo.Forge implements IFluidUpdateHelper
    {
        public Forge(@Nonnull final IBlockAccess accessIn, @Nonnull final BlockPos originIn, @Nonnull final FluidState originStateIn, final int radius, @Nonnull final Map<Block, Boolean> displacementsIn) {
            super(accessIn, originIn, originStateIn, radius, displacementsIn);
        }
    }
    ...
    default int calculateFlowCostI(final int xi, final int yi, final int zi, final int quantaPerBlock, final int flowMeta, final int flowCost, final int recurseDepth, @Nonnull final IntUnaryOperator downMeta, @Nonnull final IntSet checkedX, @Nonnull final IntSet checkedZ) {
        ...
        for(int adjSide = 0; adjSide < 4; adjSide++) {
            ...
            if((!FluidloggedUtils.isCompatibleFluid(fluid, getOrigin()) || !fluid.isSource()) && canFlowIntoI(xi, yi, zi, cappedLvl, side, true)) {
                if(canFlowIntoI(xio, yi, zio, downMeta.applyAsInt(cappedLvl), getOrigin().getDownDensityFace(), true)) return recurseDepth;
    ...
    default boolean[] getOptimalFlowDirectionsI(final int xi, final int yi, final int zi, final int quantaPerBlock, final int flowMeta, final int flowCost, @Nonnull final IntUnaryOperator downMeta) {
        ...
        for(int sideI = 0; sideI < 4; sideI++) {
            ...
            @Nonnull final FluidState fluid = getFluidStateI(xio, yi, zio);
            if((!FluidloggedUtils.isCompatibleFluid(fluid, getOrigin()) || !fluid.isSource()) && canFlowIntoI(xi, yi, zi, flowMeta, side, true)) {
                ...
                if(canFlowIntoI(xio, yi, zio, downMeta.applyAsInt(flowMeta), getOrigin().getDownDensityFace(), true)) adjFlowCost[sideI] = 0;
    ...
    default void flowIntoI(final int xi, final int yi, final int zi, final int level, @Nonnull final EnumFacing sideToCheck, final int blockFlags) {
        if(level >= 0 && canFlowIntoI(xi, yi, zi, level, sideToCheck, false)) {
            ...
            if(!isCompatibleFluidI(xio, yio, zio)) { // don't flow into compatible fluids, they will update on their own to the proper level
                ...
                if(vaporize || isReplaceableI(xio, yio, zio, getOrigin().withLevel(level), sideToCheck.getOpposite(), true, false)) {
                    ...
                    if(!vaporize && oldState.getBlock().getHarvestTool(oldState) == null)
                        oldState.getBlock().dropBlockAsItem(getCache().getWorld(), getPosIB(xio, yio, zio), oldState, 0);

                    getCache().getWorld().setBlockState(getPosIB(xio, yio, zio), getOrigin().withLevel(level).getState(), blockFlags);
                }
                ...
                // if the block can be fluidlogged with this fluid
                    ...
                    FluidloggedUtils.setFluidState(getCache().getWorld(), getPosIB(xio, yio, zio), getBlockStateI(xio, yio, zio), getOrigin().withLevel(level), false, blockFlags);
                }
    ...
    default boolean vaporizeI(final int xi, final int yi, final int zi, @Nonnull final FluidState fluidToPlace, @Nullable final EnumFacing sideToCheck) {
        if(isVaporizableI(xi, yi, zi, fluidToPlace, sideToCheck)) {
            fluidToPlace.getFluid().vaporize(null, getCache().getWorld(), getPosIB(xi, yi, zi), fluidToPlace.createFluidStack()); // play serverside sound
            FluidloggedAPI.WRAPPER.sendToAllAround(new SMessageVaporizeEffects(fluidToPlace, getCache().mutablePos), new NetworkRegistry.TargetPoint(
                    getCache().getWorld().provider.getDimension(), getXB(xi) + 0.5, getYB(yi) + 0.5, getZB(zi) + 0.5, 64)
            );
    ...
    default void flowInto(final int x, final int y, final int z, final int level, @Nonnull final EnumFacing sideToCheck, final int blockFlags) {
        flowIntoI(getXI(x), getYI(y), getZI(z), level, sideToCheck, blockFlags);
    }
